...
#              FortiClient logs a lot of inforamtion and sometimes you 
#              only need to part of it.
...
def save_results(selected_keys, all_data, original_file, filter_text, exclude_filter, context_lines):
    ...
    messagebox.showinfo("Save Successful", f"Data saved to {new_filename}")  # Show a message indicating success
...
# Save checkbox states to a file
def save_checkbox_states():
    """ Save the current states of the checkboxes to a config file. """
    with open("config.txt", "w") as f:
        for key, var in checkboxes.items():
            f.write(f"{key}={var.get()}\n")  # Write each checkbox state as 'key=value'

# Load checkbox states from a file
def load_checkbox_states():
    """ Load the checkbox states from a config file if it exists. """
    if os.path.exists("config.txt"):
        with open("config.txt", "r") as f:
            return dict(line.strip().split('=') for line in f if line.strip())  # Parse the file into a dictionary
    return {}
...
def load_file():
    ...
    for key in keys_order.keys():
        checked = saved_states.get(key, "False") == "True"  # Determine if the checkbox should be checked
        ...
        if row > 20:
            row = 0
            column += 1  # Move to the next column after 20 rows
...
# GUI Setup
root = tk.Tk()
root.title("Ultimate FortiClient EMS Log Filter v.2")
...
save_button = tk.Button(root, text="Save Selected", command=save_filtered_results)
